...
export const Selectable = ({ as, className, value, onClick, onChange, colored, ...props }: SelectableProps) => {
  if (!as || as === "button")
    return (
      <Button
        className={cx("Selectable", { selected: value, colored }, className)}
        onClick={e => {
          onChange?.(!value);
          onClick?.(e);
        }}
  ...
  if (as === "div")
    return (
      <Div
        className={cx("Selectable", { selected: value }, className)}
        onClick={e => {
          onChange?.(!value);
          onClick?.(e);
        }}
